...
let%shared () =
  ...
  Eliom_registration.Any.register ~service:Os_services.action_link_service
    (Os_session.Opt.connected_fun Web2_handlers.action_link_handler);
  ...
  Eliom_registration.Action.register
    ...
    Web2_handlers.update_language_handler;
  Web2_base.App.register ~service:Os_services.main_service
    (Web2_page.Opt.connected_page
       Web2_handlers.main_service_handler);
  Web2_base.App.register
    ~service:Web2_services.about_service
    (Web2_page.Opt.connected_page
       Web2_handlers.about_handler);
  Web2_base.App.register
    ~service:Web2_services.settings_service
    (Web2_page.Opt.connected_page
       Web2_handlers.settings_handler)

let%server () =
  Eliom_registration.Ocaml.register
    ~service:Web2_services.upload_user_avatar_service
    (Os_session.connected_fun
       Web2_handlers.upload_user_avatar_handler)

(* Print more debugging information when <debugmode/> is in config file
   (DEBUG = yes in Makefile.options).
   Example of use:
   let section = Lwt_log.Section.make "Web2:sectionname"
   ...
    ...
    ignore
      [%client
        ((* Eliom_config.debug_timings := true; *)
         ...
         Lwt_log_core.add_rule "Web2*" Lwt_log_js.Debug
         (* Lwt_log_core.add_rule "*" Lwt_log_js.Debug *)
          : unit)];
    (* Lwt_log_core.add_rule "*" Lwt_log.Debug *)
    Lwt_log_core.add_rule "Web2*" Lwt_log.Debug)
